...
export class BasketService {
    static async getBasket(basketId: number): Promise<AxiosResponse<IBasket>> {
        const rb = new RequestBuilder(BASE_BACKEND_URL + `/baskets/${basketId}`, EHttpMethods.GET)
        // .includeToken(token)
    ...
    static async createBasket(userId: number): Promise<AxiosResponse<IBasket>> {
        const rb = new RequestBuilder(BASE_BACKEND_URL + '/baskets/', EHttpMethods.POST, {userId})
